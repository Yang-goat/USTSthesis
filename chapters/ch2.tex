...
推荐按以下顺序组织内容：
\begin{enumerate}
  \item 在 Word 封面中填写信息，导出到 \verb|front/| 目录下的同名 PDF；封面开关方式是在 \verb|front/cover.tex| 中注释或取消注释 \verb|\includepdf| 行。
  \item 在两个论文主入口中选择页眉类型：\verb|\USTSHeaderTypeThesis| 对应毕业论文，\verb|\USTSHeaderTypeDesign| 对应毕业设计。
  \item 在 \verb|main-thesis.tex| 或 \verb|main-thesis-with-appendix.tex| 中维护论文整体顺序（摘要、目录、正文、结论、参考文献、附录）。
  \item 在 \verb|chapters/| 下按章节拆分正文（如 \verb|introduction.tex|、\verb|ch2.tex|）。
  \item 在 \verb|appendix/code.tex| 中维护带附录版本的附录 C 代码清单。
  \item 在 \verb|模板格式来源/| 中查看学校 Word 样式文件和国标 PDF，用于人工核对格式，不要把这些来源文件作为正文内容直接引入。
  \item 将论文图片统一放入 \verb|img/|，正文中使用 \verb|img/文件名.png| 引用。
  \item 在 \verb|reference.bib| 中维护参考文献条目，在正文通过 \verb|\cite{key}| 引用。
\end{enumerate}

如果是首次使用，建议先保留模板已有结构，只替换文字和图表内容，不要一次性改动太多排版设置。

\section{交叉引用与 \texttt{cleveref}（推荐）}\label{sec:usage-crossref}
...
章节与小节引用使用 \verb|\ref|。常见写法示例如下：
\begin{itemize}
  \item 单章引用：见第\ref{chap:usage}章。
  \item 小节引用：交叉引用规则见第\ref{sec:usage-crossref}节。
  \item 混合引用：写作检查项可结合第\ref{sec:usage-crossref}节与\cref{alg:usage-opt}说明。
\end{itemize}

\section{字体与加粗示例}

模板已经统一设置正文、标题、目录和图表等位置的字体。正文中确需强调时，可使用以下写法：
\begin{itemize}
  \item 普通中文加粗：\textbf{关键结论}。
  \item 黑体加粗强调：{\heiti\bfseries 关键结论}。
  \item 英文加粗：\textbf{important result}。
\end{itemize}

其中 \verb|\textbf{中文}| 会使用宋体加粗渲染，\verb|{\heiti\bfseries ...}| 会使用黑体加粗渲染，更接近 Word 中“黑体+加粗”的效果。
...
\begin{figure}[H]
  ...
  % 正式论文可改为：\includegraphics[width=0.62\textwidth]{img/result.png}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    % 正式论文可改为：\includegraphics[width=\textwidth]{img/result-a.png}
    \caption{子图A}
...
本模板已预置 \verb|theorem|、\verb|definition|、\verb|lemma|、\verb|proposition|、\verb|corollary| 环境，可直接使用。以下示例刻意加入英文、普通数字和交叉引用，用于检查环境正文是否保持正体；数学变量仍应按数学规则显示为斜体。

\begin{equation}\label{eq:metric-triangle}
  d(x,y)\le d(x,z)+d(z,y).
\end{equation}

\begin{definition}\label{def:metric-space}
设非空集合 $X$ 上的函数 $d: X\times X\to \mathbb{R}$ 满足非负性、对称性和\cref{eq:metric-triangle}所示的三角不等式，则称 $(X,d)$ 为度量空间。此处的 Metric 123 应显示为正体。
\end{definition}

\begin{lemma}\label{lem:zero-distance}
若 $d(x,y)=0$，则 $x=y$。此处的 Lemma 456 应显示为正体。
\end{lemma}

\begin{proposition}\label{prop:limit-distance}
若 $x_n\to x$，则 $d(x_n,x)\to 0$；其中 Proposition 789 和普通数字均应显示为正体。
\end{proposition}

\begin{theorem}\label{thm:sample}
在度量空间中，收敛序列的极限唯一。该结论记为 Uniqueness Test 1，英文与数字应显示为正体。
\end{theorem}

\begin{proofenv}
设序列 $\{x_n\}$ 同时收敛到 $x$ 与 $y$。由\cref{eq:metric-triangle}有
\[
  d(x,y)\le d(x,x_n)+d(x_n,y).
\]
令 $n\to\infty$，右侧趋于 $0$，故 $d(x,y)=0$。再由\cref{lem:zero-distance}可得 $x=y$。此处的 Proof 2026 和引用编号均应显示为正体。
\end{proofenv}

\begin{corollary}\label{cor:constant-limit}
常值序列的极限唯一，该结论可由\cref{thm:sample}直接得到。此处的 Corollary 101 应显示为正体。
\end{corollary}

在正文中可写：由\cref{def:metric-space}可得基本结构；相关性质见\cref{prop:limit-distance}；关键结论见\cref{thm:sample}，其直接推论见\cref{cor:constant-limit}。

\textit{Italic context: 外层英文为斜体，但其中的\cref{eq:metric-triangle}与\cref{thm:sample}仍应保持正体。}
...
\subsubsection*{7) 重点公式强调（empheq）}
模板新增了 \verb|empheq| 数学库，适合对“关键结论”“约束组”“最终模型”做整体强调。它本质上是在 \verb|equation|、\verb|align| 等环境外层再加一层样式控制，因此不影响原有对齐与编号方式。

例如，可直接给单个关键公式加框：
\begin{empheq}[box=\fbox]{equation}\label{eq:usage-empheq-box}
  \hat{\theta}=\arg\min_{\theta\in\Theta}\mathcal{L}(\theta).
\end{empheq}

也可以给一组约束或条件加左大括号，适合 KKT 条件、状态方程组等场景：
\begin{empheq}[left=\empheqlbrace]{align}
  \nabla f(x^\star)+A^\top\lambda^\star+E^\top\nu^\star &= 0,\\
  Ax^\star-b &\le 0,\\
  \lambda^\star &\ge 0,\\
  (\lambda^\star)^\top(Ax^\star-b) &= 0.
\end{empheq}

若正文中需要突出最终结果，可写为：核心优化目标见\cref{eq:usage-empheq-box}。

\subsubsection*{8) 常用数学符号排版建议}
\begin{itemize}
  \item 向量、矩阵建议统一风格：如 \verb|\bm{x}|、\verb|\bm{A}|。
  \item 使用 \verb|\left...\right| 处理自适应括号，避免手动调大小。
  \item 范数、绝对值可写作 \verb|\lVert x\rVert_2|、\verb||x||。
  \item 重要公式务必加 \verb|\label|，正文用 \verb|\cref| 引用，如\cref{eq:usage-split,eq:usage-opt-problem,eq:usage-kkt,eq:usage-empheq-box}。
\end{itemize}

\subsection{算法示例}

\begin{algorithm}[H]
\caption{示例算法：梯度下降迭代求解}
\label{alg:usage-opt}
\KwIn{初始解 $x_0$，学习率 $\eta$，最大迭代次数 $T$，容差 $\varepsilon$}
\KwOut{近似最优解 $x^\star$}
$x \leftarrow x_0$\\
\For{$t=1$ \KwTo $T$}{
  计算梯度 $g_t=\nabla f(x)$\\
  \If{$\lVert g_t \rVert_2 < \varepsilon$}{
    \KwRet{$x$}
  }
  $x \leftarrow x - \eta g_t$\\
}
\KwRet{$x^\star \leftarrow x$}
\end{algorithm}
...
代码清单标题默认显示为“代码 1”“代码 2”这类格式。
旧的英文编号样式不会再用于代码标题。
带附录版本的完整代码示例集中放在 \verb|appendix/code.tex|，其中包含 MATLAB、Python、C++ 和 SQL 示例。

\subsection{文献引用}
正文中可直接使用 \verb|\cite{key}|。
中文期刊论文、图书、会议论文、学位论文可参考\cite{cn_journal_2024,cn_book_2021,cn_conference_2023,cn_thesis_2022}。
标准、专利、研究报告和在线资源可参考\cite{cn_standard_2020,cn_patent_2021,cn_report_2022,cn_online_2024}。
英文期刊论文示例可参考\cite{alorf_survey_2023}。
参考文献会按正文首次引用顺序输出，通常不需要手动调整 \verb|reference.bib| 中的条目先后。

本模板保留 \verb|style=gb7714-2015| 和顺序编码输出，并在样式中关闭 URL 与 DOI 输出。即使 \verb|reference.bib| 条目中保留 \verb|url| 或 \verb|doi| 字段，参考文献表也默认不显示 URL 和 DOI。
